...
###         (MT3002 - Summer 2022) at the Department of Mathematics,
###         Stockholm University.
###         (https://kurser.math.su.se/course/view.php?id=1138)
...
###
### History:
###  -- 02 Apr 2022 made 2022 edition based on 2020 file    
...
p1 <- ggplot(ts_df, aes(x=Date, y=ratio_icu_case)) + geom_line() +  
  ...
  coord_cartesian(xlim=c(as.Date("2020-04-01"), NA), ylim=c(0,0.1)) +
...
p2 <- ggplot(ts_df, aes(x=Date, y=ratio_death_icu)) + geom_line() + 
  ...
  coord_cartesian(xlim=c(as.Date("2020-04-01"), NA), ylim=c(0,10)) +
...
tests <- tests %>%
  ...
  mutate(week = str_replace(Vecka, "vecka ", ""), 
         n_tested = str_replace(`Nukleinsyrapåvisning Testade individer`," ", "")) %>% mutate_at(c("week", "n_tested"), as.numeric) %>% select(week, n_tested)
...
if (!file.exists(file_name)) {   
  ...
  #file_url <- str_c("https://github.com/robert-koch-institut/SARS-CoV-2-Nowcasting_und_-R-Schaetzung/blob/main/Archiv/Nowcast_R_", Sys.Date(),".csv")
  file_url <- str_c("https://raw.githubusercontent.com/robert-koch-institut/SARS-CoV-2-Nowcasting_und_-R-Schaetzung/main/Archiv/Nowcast_R_", Sys.Date(), ".csv")
